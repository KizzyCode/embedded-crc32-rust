...
/// Computes a CRC32C checksum (polynomial 0x1EDC6F41)
pub const fn crc32c(data: &[u8]) -> u32 {
    // The inverted start value
    let mut value = !0;

    // `for`-loops are not const, so we use `while` instead
    let mut index = 0;
    while index < data.len() {
        // Ingest the next byte
        let table = (value ^ (data[index] as u32)) & 0xFF;
        value = TABLE_CRC32C[table as usize] ^ (value >> 8);
        index += 1;
    }

    // Invert the inverted value and return it
    !value
}
